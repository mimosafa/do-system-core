...
class PrefectureRepository implements PrefectureRepositoryInterface
{
    ...
    {
        $pref = self::$prefectures[array_search($id->getValue(), self::idsIndex())] ?? null;
        if (!$pref) {
            throw new \Exception();
        }
        return new Prefecture(
            PrefectureValueId::of($pref['id']),
            PrefectureValueName::of($pref['name'])
        );
    }
    ...
    {
        $pref = self::$prefectures[array_search($name->getValue(), self::namesIndex())] ?? null;
        if (!$pref) {
            throw new \Exception();
        }
        return new Prefecture(
     ...
     * @return int[]
     */
    private static function idsIndex(): array
    {
        if (!isset(self::$ids)) {
            self::$ids = \array_column(self::$prefectures, 'id');
        }
        return self::$ids;
    }
     ...
     * @return string[]
     */
    private static function namesIndex(): array
    {
        if (!isset(self::$names)) {
            self::$names = \array_column(self::$prefectures, 'name');
        }
        return self::$names;
    }
}
